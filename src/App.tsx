...
function ProtectedRoute({ children, role }: { children: React.ReactNode; role?: 'admin' | 'student' | 'creator' | 'attendance' | 'servant' }) {
  ...
  if (role === 'servant' && !isAdmin && !isExamCreator && !isAttendanceScanner) return <Navigate to="/" />;

  if (role === 'student') {
    return <StudentLayout>{children}</StudentLayout>;
  }

  if (role === 'admin' || role === 'creator' || role === 'attendance' || role === 'servant') {
    ...
    <AnimatePresence mode="wait">
      <Routes location={location} key={location.pathname}>
        ...
        <Route path="/student/workspace" element={
          <ProtectedRoute role="student">
            <motion.div {...pageTransition} className="w-full min-h-screen"><WorkspacePortal /></motion.div>
          </ProtectedRoute>
        } />
