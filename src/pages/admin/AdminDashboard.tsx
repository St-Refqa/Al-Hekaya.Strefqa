...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (!isListView) {
      ...
      // Total Submissions & Points
      const subsQ = query(collection(db, "submissions"), orderBy("createdAt", "desc"), limit(5));
      const unsubscribeSubs = onSnapshot(subsQ, (snapshot) => {
        const subs = snapshot.docs.map(d => ({ id: d.id, ...d.data() }) as Submission);
        setRecentSubmissions(subs);
      });

      // Overall stats (can be heavy, maybe just a count for now or cached)
      getDocs(collection(db, "submissions")).then(snap => {
        let points = 0;
        snap.docs.forEach(d => points += (d.data().finalScore || 0));
        setStats(prev => ({ 
          ...prev, 
    ...
    <div className={cn("max-w-7xl mx-auto px-4 sm:px-6 py-6 lg:py-12", i18n.language === 'ar' ? 'text-right' : 'text-left')} dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-12">
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <NotificationBell userId={useAuth().user?.uid} userRole="admin" />
          <button
          ...
          >
            ...
            {t('admin_dashboard.broadcast_title')}
          </button>
          <Link
            to="/admin/create"
      ...
      {!isListView ? (
        <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: Users, label: "إجمالي الطلاب", value: stats.totalUsers, color: "text-blue-600", bg: "bg-blue-50" },
              { icon: ClipboardCheck, label: "إجمالي التسليمات", value: stats.totalSubmissions, color: "text-emerald-600", bg: "bg-emerald-50" },
              { icon: BookOpen, label: "الاختبارات النشطة", value: stats.activeAssessments, color: "text-amber-600", bg: "bg-amber-50" },
              { icon: Star, label: "إجمالي النقاط", value: stats.totalPoints, color: "text-brand-red", bg: "bg-rose-50" }
            ].map((stat, i) => (
              <motion.div
                key={i}
                ...
                className="bg-white p-8 rounded-[40px] border border-brand-beige/10 shadow-sm flex flex-col items-center text-center group hover:shadow-xl hover:shadow-brand-red/5 transition-all"
              >
                    ...
                    recentSubmissions.map((sub) => (
                      <div key={sub.id} className="p-6 flex items-center justify-between hover:bg-brand-cream/10 transition-all">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h5 className="font-black text-brand-text leading-none">{sub.participantName}</h5>
                        ...
                        <div className="text-right">
                          <div className="text-sm font-black text-emerald-600">%{((sub.finalScore / sub.maxScore) * 100).toFixed(0)} دقة</div>
                  ...
                  <div className="p-8 relative z-10">
                    <div className="flex items-center justify-between mb-8">
                      <div className="flex items-center gap-2">
                        <span
                          className={cn(
                            "px-4 py-1.5 text-[8px] font-black uppercase rounded-full tracking-[0.2em]",
