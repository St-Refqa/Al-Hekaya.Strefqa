...
export default function AssessmentCreator() {
  ...
  const handleAddManualQuestion = () => {
    ...
    const newQ: Question = {
      ...
      modelAnswer: manualQuestion.type === "short-answer" ? manualQuestion.modelAnswer.trim() : undefined,
      aiRubric: manualQuestion.type === "short-answer" ? manualQuestion.aiRubric?.trim() : undefined,
      isLocked: true,
      isReviewed: true,
    };
    ...
    if (id) {
      const fetchAssessment = async () => {
        try {
          ...
          if (docSnap.exists()) {
            setFormData({ id: docSnap.id, ...docSnap.data() } as Assessment);
          }
        } catch (err) {
  ...
  const handleSave = async (e: React.FormEvent, publish: boolean = false) => {
    ...
    if (publish) {
      const unreviewed = allQuestions.filter((q) => !q.isReviewed);
      if (unreviewed.length > 0) {
        setError(
          `${unreviewed.length} questions need review before publishing.`,
        );
        return;
      }
    }
    ...
    const executeSave = async (finalId?: string, finalVersion: number = 1) => {
      ...
      try {
        ...
        cleanQuestions.easy = cleanQuestions.easy.map((q) => ({
          ...q,
          points: 2,
          difficulty: "easy" as const,
          isReviewed: q.isReviewed ?? false,
        }));
        cleanQuestions.medium = cleanQuestions.medium.map((q) => ({
          ...q,
          points: 4,
          difficulty: "medium" as const,
          isReviewed: q.isReviewed ?? false,
        }));
        cleanQuestions.hard = cleanQuestions.hard.map((q) => ({
          ...q,
          points: 6,
      ...
      if (id) {
        ...
        if (!subSnap.empty) {
          setConfirmModal({
            ...
            cancelText: "تعديل النسخة الحالية",
            type: "warning",
    ...
    <div className="max-w-5xl mx-auto px-6 py-12 rtl" dir="rtl">
      {!isAdmin && isExamCreator && (
        <div className="mb-8 p-6 bg-gradient-to-br from-white to-brand-cream/40 border-r-4 border-l border-t border-b border-r-brand-red border-brand-beige/10 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-6 text-right shadow-sm shadow-brand-beige/5">
          ...
          <button
            ...
            className="px-6 py-3.5 bg-brand-red text-white hover:bg-brand-red/90 text-xs font-black rounded-2xl transition-all shadow-lg shadow-brand-red/10 flex items-center gap-2 whitespace-nowrap group hover:scale-[1.02] active:scale-[0.98]"
          >
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                ...
                {confirmModal.title === "تحديث الاختبار" ? (
                   <button
                    onClick={() => {
                        // This matches the "Modify Direct" logic from before
                        // We need to pass the ID and Version to executeSave
                        const finalVersion = formData.version || 1;
                        executeSave(id, finalVersion);
                    }}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
        <div className="flex items-center gap-6">
          ...
          >
            <ArrowLeft className="w-5 h-5 text-brand-beige rotate-180" />
          </button>
          <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            disabled={isSaving}
            className="px-6 py-4 bg-white border border-brand-beige/20 text-brand-text rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-brand-cream transition-all shadow-sm flex items-center gap-2"
          >
