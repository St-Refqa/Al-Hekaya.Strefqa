...
export default function ResultsDashboard() {
  ...
  useEffect(() => {
    ...
    // Fetch user photos to ensure they show even for old submissions
  ...
  const handleScoreUpdate = async (
    ...
    const newFinalScore =
      newAnswers.reduce((acc, curr) => acc + curr.score, 0) + (sub.bonusPoints || 0) + (sub.adminAdjustment || 0);
  ...
  const handleAdminAdjustment = async (sub: Submission, amount: number, reason: string) => {
    ...
    const newFinalScore = baseScore + (sub.bonusPoints || 0) + amount;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <SummaryItem
                      label="بونص الالتزام"
                      value={`+${submission.bonusPoints || 0}`}
                      sub="يتم حسابه تلقائياً"
                    />
