...
export default function AdminAnalytics() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const [aSnap, sSnap, uSnap] = await Promise.all([
          getDocs(collection(db, "assessments")),
          getDocs(collection(db, "submissions")),
          getDocs(query(collection(db, "users"), where("role", "==", "student")))
        ]);

        setAssessments(aSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Assessment)));
        setSubmissions(sSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Submission)));
        setStudents(uSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as User)));
      } catch (err) {
