...
const StudentDetail: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'exams' | 'store' | 'points' | 'logins'>('overview');
    ...
    const fetchStudentData = async () => {
      ...
      try {
        ...
        if (studentDoc.exists()) {
          setStudent({ uid: studentDoc.id, ...studentDoc.data() } as User);
        }
        ...
        setSubmissions(subSnap.docs.map(d => {
          const data = d.data();
          return { 
            id: d.id, 
            ...data,
        ...
        // Fetch Point Logs (Only if collection exists in rules)
        try {
          ...
          setPointLogs(pointsSnap.docs.map(d => ({ id: d.id, ...d.data() } as PointLog)));
        } catch (e) {
        ...
        setLoginLogs(loginSnap.docs.map(d => ({ 
          id: d.id, 
          ...d.data(),
    ...
    return (
      <div className="min-h-screen bg-brand-cream flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-brand-beige">جاري تحميل بيانات الطالب...</p>
        </div>
      </div>
    );
  }
  ...
  if (!student) {
    return (
      <div className="min-h-screen bg-brand-cream p-8 text-center flex flex-col items-center justify-center gap-6">
        <h2 className="text-3xl font-black text-brand-text">الطالب غير موجود</h2>
        <Link to="/admin/users" className="px-8 py-4 bg-brand-red text-white rounded-2xl font-bold hover:scale-105 transition-transform">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
            ...
            <div className="flex items-center gap-3 md:gap-4 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-1.5 md:gap-3">
                  <h1 className="text-xl md:text-2xl font-black text-brand-text truncate max-w-xs md:max-w-none">{student.fullName}</h1>
                  {student.code?.toUpperCase().startsWith('H') && (
                    <span className="px-2 md:px-3 py-0.5 md:py-1 bg-blue-50 text-blue-600 rounded-full text-[9px] md:text-[10px] font-black uppercase whitespace-nowrap">عهد قديم</span>
                  )}
                  {student.code?.toUpperCase().startsWith('N') && (
                    <span className="px-2 md:px-3 py-0.5 md:py-1 bg-purple-50 text-purple-600 rounded-full text-[9px] md:text-[10px] font-black uppercase whitespace-nowrap">عهد جديد</span>
                  )}
                  {student.code?.toUpperCase().startsWith('S') && (
                    <span className="px-2 md:px-3 py-0.5 md:py-1 bg-amber-50 text-amber-600 rounded-full text-[9px] md:text-[10px] font-black uppercase whitespace-nowrap">خادم</span>
                  )}
                </div>
                <p className="text-brand-beige font-bold text-xs md:text-sm mt-0.5">كود الطالب: {student.code}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto justify-end">
            <div className="flex-1 md:flex-initial px-4 md:px-6 py-2.5 md:py-3 bg-brand-cream border border-brand-beige/20 rounded-xl flex items-center justify-center gap-2 md:gap-3">
              <span className="text-[9px] md:text-[10px] font-black text-brand-beige uppercase">النقاط</span>
              <span className="text-lg md:text-xl font-black text-brand-red">{student.totalPoints || 0}</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-6 md:py-10">
        ...
        <div className="flex items-center gap-1.5 md:gap-2 bg-white/50 p-1.5 md:p-2 rounded-2xl md:rounded-[24px] border border-brand-beige/10 mb-6 md:mb-10 overflow-x-auto hide-scrollbar scroll-smooth">
          {tabs.map((tab) => (
            ...
            >
              <tab.icon className={cn("w-4 h-4 md:w-5 md:h-5 shrink-0", activeTab === tab.id ? "text-brand-red" : "text-brand-beige")} />
              <span>{tab.label}</span>
            </button>
          ))}
        </div>
              ...
              >
                ...
                <div className="md:col-span-2 bg-white rounded-[40px] p-8 border border-brand-beige/10 shadow-sm space-y-8">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xl font-black text-brand-text">بيانات الطالب الشخصية</h3>
                    {student.photoUrl && (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-brand-beige uppercase">الاسم بالكامل</p>
                        <p className="font-bold text-brand-text">{student.fullName}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-brand-beige uppercase">الكنيسة</p>
                        <p className="font-bold text-brand-text">{student.church || "غير مسجل"}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-brand-beige uppercase">رقم الواتساب</p>
                        <p className="font-bold text-brand-text">{student.whatsappNumber || "غير متوفر"}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-brand-beige uppercase">تاريخ الميلاد</p>
                        <p className="font-bold text-brand-text">{student.birthDate ? formatDate(student.birthDate) : "غير مسجل"}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-brand-beige uppercase">كود الطالب</p>
                        <p className="font-bold text-brand-text">{student.code}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-brand-beige uppercase">كلمة المرور</p>
                        <p className="font-bold text-brand-text">{student.password}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 sm:col-span-2">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-brand-beige uppercase">العنوان</p>
                        <p className="font-bold text-brand-text">{student.address || "غير مسجل"}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 sm:col-span-2">
                      ...
                      <div>
                        <p className="text-[10px] font-black text-brand-beige uppercase">تاريخ التسجيل</p>
                        <p className="font-bold text-brand-text">{student.registrationDate ? formatDate(student.registrationDate) : "غير متوفر"}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6 content-start">
                  <div className="bg-white rounded-[32px] p-8 border border-brand-beige/10 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <div>
                      <p className="text-4xl font-black text-brand-text">{submissions.length}</p>
                  ...
                  <div className="bg-white rounded-[32px] p-8 border border-brand-beige/10 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-brand-beige">متوسط التقييم</p>
                    </div>
                  </div>
                  <div className="bg-white rounded-[32px] p-8 border border-brand-beige/10 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <div>
                      <p className="text-4xl font-black text-brand-text">{student.totalPoints || 0}</p>
                      <p className="font-bold text-brand-beige">النقاط الحالية</p>
                    </div>
                  </div>
                  <div className="bg-white rounded-[32px] p-8 border border-brand-beige/10 flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <div>
                      <p className="text-4xl font-black text-brand-text">{purchases.length}</p>
                      <p className="font-bold text-brand-beige">طلب من المتجر</p>
                    </div>
                  </div>
                </div>
              </motion.div>
                ...
                <div className="bg-white rounded-[32px] overflow-hidden border border-brand-beige/10">
                  <table className="w-full text-right">
                    ...
                    <tbody className="divide-y divide-brand-beige/10">
                      {submissions.map((sub) => (
                        <tr key={sub.id} className="hover:bg-brand-cream/30 transition-colors">
                          <td className="px-8 py-6">
                            <p className="font-black text-brand-text">{sub.assessmentTitle}</p>
                          </td>
                          <td className="px-8 py-6 text-brand-beige font-bold text-sm">
                            {formatDate(sub.createdAt)}
                          </td>
                          <td className="px-8 py-6">
                            <span className="font-black text-brand-red">{sub.score}</span>
                            <span className="text-brand-beige font-bold text-xs"> / {sub.totalPoints}</span>
                          </td>
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-2 bg-brand-cream rounded-full overflow-hidden">
                                <div 
                                  className={cn(
                                    "h-full",
                                    sub.percentage >= 80 ? "bg-green-500" : sub.percentage >= 50 ? "bg-yellow-500" : "bg-red-500"
                                  )}
                                  style={{ width: `${sub.percentage}%` }}
                                />
                              </div>
                              <span className="font-black text-brand-text">{sub.percentage}%</span>
                            </div>
                          </td>
                      ...
                      {submissions.length === 0 && (
                        <tr>
                          <td colSpan={5} className="px-8 py-20 text-center text-brand-beige font-bold">
                    ...
                    <tbody className="divide-y divide-brand-beige/10">
                      {purchases.map((order) => (
                        <tr key={order.id} className="hover:bg-brand-cream/30 transition-colors">
                          <td className="px-8 py-6">
                            <p className="font-black text-brand-text">{order.itemTitle}</p>
                          </td>
                          <td className="px-8 py-6 font-bold text-brand-text">
                            {order.quantity}
                          </td>
                          <td className="px-8 py-6">
                            <span className="font-black text-brand-red">{order.price}</span>
                            <span className="text-brand-beige font-bold text-xs mr-1">نقطة</span>
                          </td>
                          <td className="px-8 py-6 text-brand-beige font-bold text-sm">
                          ...
                          <td className="px-8 py-6">
                            <span className={cn(
                              ...
                              {order.status === 'pending' ? "قيد الانتظار" : 
                               order.status === 'completed' ? "تم التسليم" : "مرفوض"}
                            </span>
                          </td>
                        </tr>
                      ))}
              ...
              >
                <div className="bg-white rounded-[32px] overflow-hidden border border-brand-beige/10">
                   <div className="p-8 border-b border-brand-beige/10 flex items-center justify-between">
                     <h3 className="text-xl font-black text-brand-text">سجل حركات النقاط</h3>
                   ...
                   <div className="divide-y divide-brand-beige/10">
                     {pointLogs.map((log) => (
                       <div key={log.id} className="p-6 flex items-center justify-between hover:bg-brand-cream/30 transition-colors">
                         <div className="flex items-center gap-4">
                           <div className={cn(
                             "w-10 h-10 rounded-full flex items-center justify-center shrink-0",
                             log.type === 'add' ? "bg-green-50 text-green-600" : "bg-red-50 text-brand-red"
                           )}>
                             {log.type === 'add' ? "+" : "-"}
                           </div>
                           <div>
                             <p className="font-bold text-brand-text">{log.reason}</p>
                             <p className="text-xs text-brand-beige font-bold">{formatDate(log.createdAt)}</p>
                           </div>
                         </div>
                         <div className={cn(
                           "text-lg font-black",
                           log.type === 'add' ? "text-green-600" : "text-brand-red"
                         )}>
              ...
              >
                <div className="bg-white rounded-[32px] overflow-hidden border border-brand-beige/10">
                  <table className="w-full text-right">
                    ...
                    <tbody className="divide-y divide-brand-beige/10">
                      {loginLogs.map((log) => (
                        <tr key={log.id} className="hover:bg-brand-cream/30 transition-colors text-sm">
                          <td className="px-8 py-6 font-bold text-brand-text">
                      ...
                      {loginLogs.length === 0 && (
                        <tr>
                          <td colSpan={3} className="px-8 py-20 text-center text-brand-beige font-bold">
