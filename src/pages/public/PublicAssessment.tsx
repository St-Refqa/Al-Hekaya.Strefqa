...
export default function PublicAssessment() {
  ...
  const completeAssessment = useCallback(async (finalAnswers: UserAnswer[], auto: boolean = false) => {
    ...
    // Unanswered count
    const unansweredCount = 3 - finalAnswers.length;
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        if (participantSnap.exists()) {
          ...
          if (streakCount % 7 === 0 && pData.streakCount !== streakCount) {
            bonusPoints = 5;
          }
          ...
          transaction.set(participantRef, {
            ...
            totalBonusPoints: (pData.totalBonusPoints || 0) + bonusPoints,
          }, { merge: true });
        } else {
          transaction.set(participantRef, {
            name: participantName,
        ...
        if (userRef && userSnap?.exists()) {
          ...
          const newTotalPoints = (uData.totalPoints || 0) + baseScore + bonusPoints;
          ...
          const xpGained = (baseScore * 5) + (streakCount * 5) + (bonusPoints * 10);
          ...
          notificationService.sendNotification({
            ...
            message: `لقد أكملت اختبار ${assessment?.title} بنجاح. درجتك: ${baseScore + bonusPoints}`,
            type: "info",
    ...
    if (saved) {
      try {
        ...
        const restore = () => {
          ...
          } else if (savedPhase === "ANSWERING" || savedPhase === "QUESTION" || savedPhase === "DIFFICULTY_SELECTOR") {
  ...
  // Save persistence
  useEffect(() => {
    if (phase === "READING" || phase === "DIFFICULTY_SELECTOR" || phase === "QUESTION") {
  ...
  useEffect(() => {
    ...
    } else if ((phase === "DIFFICULTY_SELECTOR" || phase === "QUESTION") && answeringTimeLeft !== null && answeringTimeLeft > 0) {
      interval = setInterval(() => {
    ...
    if (answeringTimeLeft === 0 && (phase === "DIFFICULTY_SELECTOR" || phase === "QUESTION")) {
  ...
  }, [phase, readingTimeLeft, answeringTimeLeft, assessment, selectedAnswers, autoSubmit]);
  ...
  const handleDifficultyChoice = (difficulty: "easy" | "medium" | "hard") => {
    setSelectedDifficulty(difficulty);
    const pool = assessment!.questions[difficulty];
    // Find a question not already used
    const usedIds = selectedAnswers.map((a) => a.questionId);
    const available = pool.filter((q) => !usedIds.includes(q.id));

    if (available.length === 0) {
      // Fallback/Safety: just pick first if none available (shouldn't happen with 3 each)
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === "DIFFICULTY_SELECTOR" && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="max-w-2xl mx-auto px-6 py-24 h-screen flex flex-col justify-center"
          >
            <div className="text-center space-y-4 mb-12">
              <span className="px-3 py-1 bg-brand-red/5 text-brand-red rounded-full text-[10px] font-black uppercase tracking-widest">
                سؤال {currentQuestionIndex + 1} من 3
              </span>
              <h2 className="text-4xl font-extrabold text-brand-text tracking-tight">
                اختار مستوى الصعوبة
              </h2>
              <p className="text-brand-text/60 font-bold tracking-tight">
                اختيارك بيحدد عدد الدرجات وصعوبة تقييم الذكاء الاصطناعي.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              <DifficultyCard
                level="سهل"
                points={2}
                description="أسئلة مباشرة وتذكر للمعلومات."
                color="green"
              ...
              <DifficultyCard
                level="متوسط"
                points={4}
                description="أسئلة بتحتاج فهم أعمق وتحليل للنص."
                color="gold"
              ...
              <DifficultyCard
                level="صعب"
                points={6}
                description="أسئلة نقدية وربط للمعلومات ببعضها."
                color="red"
        ...
        {phase === "RESULTS" && finalSubmission && (
          ...
          >
            <div className="bg-white p-16 rounded-[48px] shadow-2xl space-y-16 border border-brand-beige/20">
              ...
              {finalSubmission.bonusPoints > 0 && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="bg-brand-red p-8 rounded-[40px] text-white flex items-center justify-between shadow-xl shadow-brand-red/20"
                >
                  <div className="flex items-center gap-6">
                    <Trophy className="w-12 h-12 opacity-80" />
                    <div className="text-right">
                      <h4 className="font-black text-[10px] uppercase tracking-[0.2em] opacity-80 mb-1">
                        بونص الالتزام
                      </h4>
                      <p className="text-xl font-bold">
                        زودنا لك 5 درجات عشان أنت ملتزم وبقالك 7 أيام بتمتحن.
                      </p>
                    </div>
                  </div>
                  <div className="text-5xl font-light">
                    +{finalSubmission.bonusPoints}
                  </div>
                </motion.div>
              )}
