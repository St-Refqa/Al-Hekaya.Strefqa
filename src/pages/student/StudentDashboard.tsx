          ...
          <section className="mb-10">
            <div className="flex items-center justify-between mb-4 px-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg md:text-2xl font-black text-brand-text">لوحة الـ Streak 🔥</h3>
                  <p className="text-[10px] font-bold text-brand-beige uppercase tracking-widest">من اكتر واحد ثابت ومنتظم!</p>
                </div>
              </div>
              <Link to="/student/leaderboard" className="text-brand-red text-xs font-black uppercase tracking-widest hover:underline">
                كل المتصدرين
              </Link>
            </div>

            <div className="bg-white rounded-3xl md:rounded-[40px] border border-brand-beige/10 shadow-sm overflow-hidden">
              {/* Top 3 Podium */}
              {streakLeaderboard.length >= 1 && (
                <div className="bg-gradient-to-br from-orange-50 via-amber-50/50 to-white p-6 md:p-8 border-b border-orange-100/60">
                  <div className="flex items-end justify-center gap-3 md:gap-6">
                    ...
                    {streakLeaderboard[1] && (
                      ...
                      >
                        <div className="relative">
                          <div className={cn(
                            "w-14 h-14 md:w-20 md:h-20 rounded-2xl md:rounded-3xl border-4 bg-slate-100 flex items-center justify-center font-black text-xl md:text-2xl overflow-hidden shadow-lg",
                            streakLeaderboard[1].uid === user?.uid ? "border-brand-red" : "border-slate-300"
                          )}>
                            {streakLeaderboard[1].photoUrl
                              ? <img src={streakLeaderboard[1].photoUrl} alt="" className="w-full h-full object-cover" />
                              : <span className="text-slate-500">{streakLeaderboard[1].fullName[0]}</span>}
                          </div>
                          <div className="absolute -top-2 -right-2 w-6 h-6 bg-slate-400 text-white rounded-lg flex items-center justify-center font-black text-xs shadow-md border-2 border-white">2</div>
                        </div>
                        <p className="font-black text-brand-text text-xs text-center truncate max-w-[70px] md:max-w-[90px]">
                          {streakLeaderboard[1].uid === user?.uid ? 'أنت 😊' : streakLeaderboard[1].fullName.split(' ')[0]}
                        </p>
                        <div className="flex items-center gap-1 bg-slate-100 px-2.5 py-1 rounded-full">
                          <Flame className="w-3 h-3 text-orange-400" />
                          <span className="font-black text-slate-600 text-xs">{streakLeaderboard[1].streak}</span>
                        </div>
                      </motion.div>
                    )}
                    ...
                    >
                      ...
                      >
                        <Crown className="w-7 h-7 drop-shadow-[0_0_8px_rgba(251,191,36,0.6)]" />
                      </motion.div>
                      <div className="relative">
                        <div className={cn(
                          "w-20 h-20 md:w-28 md:h-28 rounded-3xl border-4 bg-amber-50 flex items-center justify-center font-black text-2xl md:text-3xl overflow-hidden shadow-xl",
                          streakLeaderboard[0].uid === user?.uid ? "border-brand-red" : "border-amber-400"
                        )}>
                          {streakLeaderboard[0].photoUrl
                            ? <img src={streakLeaderboard[0].photoUrl} alt="" className="w-full h-full object-cover" />
                            : <span className="text-amber-600">{streakLeaderboard[0].fullName[0]}</span>}
                        </div>
                        <motion.div
                      ...
                      <p className="font-black text-brand-text text-sm text-center truncate max-w-[80px] md:max-w-[110px]">
                        {streakLeaderboard[0].uid === user?.uid ? 'أنت 🏆' : streakLeaderboard[0].fullName.split(' ')[0]}
                      </p>
                      <div className="flex items-center gap-1.5 bg-amber-100 px-3 py-1.5 rounded-full shadow-sm">
                        <Flame className="w-3.5 h-3.5 text-orange-500" />
                        <span className="font-black text-amber-700 text-sm">{streakLeaderboard[0].streak} يوم</span>
                      </div>
                    </motion.div>
                    ...
                    {streakLeaderboard[2] && (
                      ...
                      >
                        <div className="relative">
                          <div className={cn(
                            "w-12 h-12 md:w-16 md:h-16 rounded-2xl border-4 bg-amber-50 flex items-center justify-center font-black text-lg overflow-hidden shadow-lg",
                            streakLeaderboard[2].uid === user?.uid ? "border-brand-red" : "border-amber-300"
                          )}>
                            {streakLeaderboard[2].photoUrl
                              ? <img src={streakLeaderboard[2].photoUrl} alt="" className="w-full h-full object-cover" />
                              : <span className="text-amber-700">{streakLeaderboard[2].fullName[0]}</span>}
                          </div>
                          <div className="absolute -top-2 -right-2 w-6 h-6 bg-amber-600 text-white rounded-lg flex items-center justify-center font-black text-xs shadow-md border-2 border-white">3</div>
                        </div>
                        <p className="font-black text-brand-text text-xs text-center truncate max-w-[60px] md:max-w-[80px]">
                          {streakLeaderboard[2].uid === user?.uid ? 'أنت' : streakLeaderboard[2].fullName.split(' ')[0]}
                        </p>
                        <div className="flex items-center gap-1 bg-amber-50 px-2.5 py-1 rounded-full">
                          <Flame className="w-3 h-3 text-orange-400" />
                          <span className="font-black text-amber-700 text-xs">{streakLeaderboard[2].streak}</span>
                        </div>
                      </motion.div>
                    )}
                  </div>
              ...
              {streakLeaderboard.length > 3 && (
                <div className="divide-y divide-brand-beige/5">
                  {streakLeaderboard.slice(3).map((s, idx) => (
                    <motion.div
                      key={s.uid}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      ...
                      className={cn(
                        "flex items-center gap-3 md:gap-5 px-5 md:px-8 py-3.5 md:py-4 hover:bg-brand-cream/10 transition-colors",
                        s.uid === user?.uid && "bg-orange-50/60 border-y border-orange-100"
                      )}
                    >
                      {/* Rank */}
                      <div className="w-7 text-center font-black text-brand-beige text-sm shrink-0">{idx + 4}</div>

                      {/* Avatar */}
                      <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-brand-cream border-2 border-white shadow-sm flex items-center justify-center text-brand-red font-black text-sm overflow-hidden shrink-0">
                        {s.photoUrl
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-black text-brand-text text-sm truncate">
                          {s.fullName}
                          {s.uid === user?.uid && <span className="mr-2 text-[9px] px-1.5 py-0.5 bg-brand-red text-white rounded-full">أنت</span>}
                        </p>
                      </div>

                      {/* Streak badge */}
                      <div className="flex items-center gap-1.5 bg-orange-50 border border-orange-100 px-3 py-1.5 rounded-xl shrink-0">
                        <Flame className="w-3.5 h-3.5 text-orange-500" />
                        <span className="font-black text-orange-600 text-sm">{s.streak}</span>
                        <span className="text-[9px] font-bold text-orange-400">يوم</span>
                      </div>
                    </motion.div>
                  ))}
                </div>
              ...
              {streakLeaderboard.length > 0 && !streakLeaderboard.find(s => s.uid === user?.uid) && currentStreak > 0 && (
                <div className="border-t border-brand-beige/10 px-5 md:px-8 py-4 bg-orange-50/40 flex items-center gap-3">
                  <Zap className="w-4 h-4 text-orange-400 shrink-0" />
                  <p className="text-xs font-black text-orange-600">
                    streak-ك الحالي: <span className="text-orange-700">{currentStreak} يوم</span> — استمر عشان تدخل القائمة! 🔥
                  </p>
                </div>
              )}
            </div>
