...
export default function AvailableAssessments() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      let filtered = data.filter(a => new Date(a.expiresAt) > new Date());
      
      // Filter based on target group
      if (user) {
        filtered = filtered.filter(a => {
          if (!a.targetGroup || a.targetGroup === 'all') return true;
          if (a.targetGroup === 'admin' && user.role === 'admin') return true;
    ...
    <div className="min-h-screen bg-[#FDFCFB] pb-24" dir="rtl">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 lg:py-12">
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {isLoading ? (
              ...
              >
                ...
                <h3 className="text-xl font-black text-brand-text">{t('assessments.no_results')}</h3>
              ...
              filteredAssessments.map((assessment, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className={cn(
                      "group cursor-pointer bg-white border border-brand-beige/10 hover:border-brand-red/30 transition-all duration-500 relative overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-brand-red/10",
                      viewMode === "grid" 
                  ...
                  >
                    ...
                    <div className={cn(
                      ...
                      <div className={cn(
                        ...
                        viewMode === "grid" 
                          ? "w-14 h-14 md:w-20 md:h-20 bg-brand-cream text-brand-red mb-4 md:mb-8 group-hover:bg-brand-red group-hover:text-white group-hover:rotate-12 group-hover:scale-110" 
                          : "w-12 h-12 md:w-16 md:h-16 bg-brand-cream text-brand-red group-hover:bg-brand-red group-hover:text-white"
                      )}>
                        <BookOpen className={viewMode === "grid" ? "w-6 h-6 md:w-8 md:h-8" : "w-5.5 h-5.5 md:w-7 md:h-7"} />
                      </div>
                      ...
                      <div className="flex-1 space-y-4">
                        ...
                        <div className="space-y-1">
                          <h3 className="text-lg md:text-2xl font-black text-brand-text leading-tight group-hover:text-brand-red transition-colors line-clamp-2">
                            {assessment.title}
                          </h3>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 pt-2 border-t border-brand-beige/5">
                          ...
                          <div className="flex items-center gap-2 text-brand-beige text-[11px] font-bold">
                            ...
                            <span>{assessment.questions.length} {t('admin.questions_count')}</span>
                          </div>
                          <div className="flex items-center gap-2 text-brand-beige text-[11px] font-bold">
                    ...
                    <div className={cn(
                      ...
                      <div className={cn(
                        ...
                        viewMode === "grid" 
                          ? "bg-brand-cream/50 group-hover:bg-brand-red rounded-2xl md:rounded-3xl p-3 md:p-4 transition-all duration-300 shadow-inner" 
                          : "gap-4"
                      )}>
                        <span className={cn(
                          "text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] transition-colors",
                          viewMode === "grid" 
                            ? "text-brand-text group-hover:text-white px-2 md:px-4" 
                            : "text-brand-beige group-hover:text-brand-red"
                        )}>
        ...
        <div className="mt-20 text-center space-y-4 opacity-40">
           <div className="w-12 h-1 bg-brand-beige/20 mx-auto rounded-full" />
           <p className="text-[10px] font-black text-brand-beige uppercase tracking-[0.4em]">تحت إشراف مدرسة سانت رفقة</p>
        </div>
      </main>
    </div>
  );
