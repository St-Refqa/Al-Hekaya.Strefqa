             ...
             <div className="absolute top-full mt-2 left-0 right-0 bg-white p-4 rounded-2xl shadow-xl border border-brand-beige/10 opacity-0 group-hover:opacity-100 transition-opacity z-20 pointer-events-none">
                <div className="space-y-2 text-xs">
                   ...
                   {submission.bonusPoints > 0 && (
                     <div className={cn("flex justify-between items-center font-bold text-emerald-600", i18n.language === 'ar' ? 'flex-row-reverse' : 'flex-row')}>
                        <span className="text-brand-beige">{t('assessments.bonus_points')}:</span>
                        <span>+{submission.bonusPoints}</span>
                     </div>
                   )}
